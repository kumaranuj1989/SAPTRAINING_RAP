...
CLASS lcl_dbtable DEFINITION.
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS c_template TYPE tabname             VALUE 'YDEMO_TRAVEL'.
    CONSTANTS c_user     TYPE abp_lastchange_user VALUE 'MENTORSAP'.

    TYPES     tt_data TYPE STANDARD TABLE OF ydemo_travel
  ...
  METHOD constructor.
    ...
    TRY.
        cl_abap_dyn_prg=>check_table_name_tab(
                 val               = i_tabname
                 packages          = VALUE #( ( `ZLOCAL` ) (  `YDEMO_MENTOR` )  )
...
* check for compatibility of type
  ...
  METHOD generate_data.
    ...
    DATA(lv_agency)   =  ycl_demo_model=>get_agency_by_user( sy-uname ).
...
* Travel in the past
     (
       agency_id   = lv_agency
       travel_id   = ycl_demo_model=>get_next_travelid( )
